...
Mat agglomerativeSegmentation(Mat original, int numClusters) {
    ...
    for (int s = 1; s <= original.rows*original.cols - numClusters ; s++) {
        ...
        clusters.at(c1) = mergeClusters(clusters.at(c1), clusters.at(c2));
        clusters.at(c2) = eraseClusters();
        diff[c1][c2] = 0;
    }

    //filter clusters

    for (int l = 0; l < original.rows*original.cols ; l++){
        if ( (clusters.at(l)[0] == -1) || (clusters.at(l)[1] == -1) || (clusters.at(l)[2] == -1) ){

            clusters.erase(clusters.begin() + l);
        }
        else {
            continue;
        }
